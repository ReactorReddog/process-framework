...
public class AuthAlipayRequest extends AuthDefaultRequest {
    ...
    protected AuthUser getUserInfo(AuthToken authToken) {
        ...
        String location = String.format("%s %s", StringUtil.isEmpty(province) ? "" : province, StringUtil.isEmpty(city) ? "" : city);

        return AuthUser.builder()
                .rawUserInfo(JSONObject.parseObject(JSONObject.toJSONString(response)))
                .uuid(response.getUserId())
                .username(StringUtil.isEmpty(response.getUserName()) ? response.getNickName() : response.getUserName())
