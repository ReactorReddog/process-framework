...
public class AuthChecker {
    ...
    public static boolean isSupportedAuth(AuthConfig config, AuthSource source) {
        boolean isSupported = StringUtil.isNotEmpty(config.getClientId()) && StringUtil.isNotEmpty(config.getClientSecret()) && StringUtil.isNotEmpty(config.getRedirectUri());
        if (isSupported && AuthDefaultSource.ALIPAY == source) {
            isSupported = StringUtil.isNotEmpty(config.getAlipayPublicKey());
        }
        if (isSupported && AuthDefaultSource.STACK_OVERFLOW == source) {
            isSupported = StringUtil.isNotEmpty(config.getStackOverflowKey());
        }
        if (isSupported && AuthDefaultSource.WECHAT_ENTERPRISE == source) {
            isSupported = StringUtil.isNotEmpty(config.getAgentId());
        }
        if (isSupported && AuthDefaultSource.CODING == source) {
            isSupported = StringUtil.isNotEmpty(config.getCodingGroupName());
        }
        if (isSupported && AuthDefaultSource.XMLY == source) {
            isSupported = StringUtil.isNotEmpty(config.getDeviceId()) && null != config.getClientOsType();
            if (isSupported) {
                isSupported = config.getClientOsType() == 3 || StringUtil.isNotEmpty(config.getPackId());
            }
        }
        return isSupported;
    }
    ...
    public static void checkState(String state, AuthSource source, AuthStateCache authStateCache) {
        ...
        if (StringUtil.isEmpty(state) || !authStateCache.containsKey(state)) {
