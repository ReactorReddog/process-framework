...
import static org.processframework.gateway.common.constant.GatewayConstant.THROWABLE_KEY;
import static org.processframework.gateway.common.constant.GatewayConstant.UNKNOWN_PATH;
...
public class ServerWebExchangeUtil {
    /**
     * 读写表单数据
     * @see <a href="https://blog.csdn.net/andy_zhang2007/article/details/100162830">FormHttpMessageConverter描述介绍</a>
     */
    ...
    public static ApiParam getApiParamByQuery(ServerWebExchange exchange, String query) {
        ...
        apiParam.setIp(ip);
        Map<String, ?> params = WebUtil.parseQueryToMap(query);
    ...
    public static ApiParam getApiParam(ServerWebExchange exchange, byte[] body) {
        ...
        if (StringUtils.containsAny(contentTypeStr, "json", "text")) {
            ...
            WebUtil.UploadInfo uploadInfo = WebUtil.getUploadInfo(fileUploadRequest);
            params = uploadInfo.getUploadParams();
            apiParam.setUploadContext(uploadInfo.getUploadContext());
        } else {
            // APPLICATION_FORM_URLENCODED请求
            params = WebUtil.parseQueryToMap(new String(body, StandardCharsets.UTF_8));
        }
        if (params != null) {
            apiParam.putAll(params);
        }
    ...
    public static ApiParam getApiParam(ServerWebExchange exchange) {
        return exchange.getAttribute(GatewayConstant.CACHE_API_PARAM);
    }
    ...
    public static void setApiParam(ServerWebExchange exchange, ApiParam apiParam) {
        exchange.getAttributes().put(GatewayConstant.CACHE_API_PARAM, apiParam);
    }
    ...
    public static HttpServletRequest getFileUploadRequest(ServerWebExchange exchange) {
        return exchange.getAttribute(GatewayConstant.CACHE_UPLOAD_REQUEST);
    }

    public static void setFileUploadRequest(ServerWebExchange exchange, HttpServletRequest request) {
        exchange.getAttributes().put(GatewayConstant.CACHE_UPLOAD_REQUEST, request);
    }
    ...
    public static <T extends Map<String, Object>> ServerWebExchange format(
        ...
        if (serverHttpRequest.getMethod() == HttpMethod.GET) {
            // 新的查询参数
            String queryString = WebUtil.convertMapToQueryString(apiParam);
            // 创建一个新的request，并使用新的uri
            newRequest = new ProcessServerHttpRequestDecorator(serverHttpRequest, queryString);
        } else {
            ...
            if (StringUtils.containsAny(contentType, "json", "text")) {
                ...
                newRequest = new ProcessServerHttpRequestDecorator(serverHttpRequest, bodyBytes);
                ...
                try {
                    ...
                    // 将字段以及上传文件重写写入到流中
                    FORM_HTTP_MESSAGE_CONVERTER.write(builder, mediaType, outputMessage);
                    // 获取新的上传文件流
                ...
                // 否则一律按表单请求处理
                String bodyStr = WebUtil.convertMapToQueryString(apiParam);
                byte[] bodyBytes = bodyStr.getBytes(StandardCharsets.UTF_8);
                newRequest = new ProcessServerHttpRequestDecorator(serverHttpRequest, bodyBytes);
            }
        }
