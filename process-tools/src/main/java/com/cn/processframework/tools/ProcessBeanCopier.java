...
public abstract class ProcessBeanCopier {
	...
	private static final Type BEAN_COPIER = TypeUtils.parseType(ProcessBeanCopier.class.getName());
	...
	/**
	 * The map to store {@link ProcessBeanCopier} of source type and class type for copy.
	 */
	private static final ConcurrentMap<ProcessBeanCopierKey, ProcessBeanCopier> BEAN_COPIER_MAP = new ConcurrentHashMap<>();

	public static ProcessBeanCopier create(Class source, Class target, boolean useConverter) {
		return ProcessBeanCopier.create(source, target, useConverter, false);
	}

	public static ProcessBeanCopier create(Class source, Class target, boolean useConverter, boolean nonNull) {
		ProcessBeanCopierKey copierKey = new ProcessBeanCopierKey(source, target, useConverter, nonNull);
	...
	public static class Generator extends AbstractClassGenerator {
		private static final Source SOURCE = new Source(ProcessBeanCopier.class.getName());
		...
		@Override
		public ProcessBeanCopier create(Object key) {
			return (ProcessBeanCopier) super.create(key);
		}
